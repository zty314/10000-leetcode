...
def del_allconf():
    ...
    for i in range(conf_sum):
        ...
        try:
            rep_delconf = requests.delete(f'http://{ip}/api/v1/mc/confs/{conf_e164}', params=params, headers=headers, cookies=cookie)
...
def get_personal_templates():
    get_confs = requests.get('http://%s/api/v1/mc/personal_templates?count=0' % ip, params=params, headers=headers, cookies=cookie)
...
def creat_conf_any(template_lists):
    ...
    for i in template_lists:
        ...
        data_createconf["params"] = json.dumps(params)
        data_createconf["account_token"] = account_token
        creat_conf_list.append(grequests.post('http://%s/api/v1/mc/confs' % ip, data=data_createconf, headers=headers, cookies=cookie))
...
def add_mt():
    ...
    data_createconf["params"] = json.dumps(params)
    data_createconf["account_token"] = account_token
    creat_conf_list.append(grequests.post(f'http://%s/api/v1/vc/confs/8888717/mts' % ip, data=data_createconf, headers=headers, cookies=cookie))
